...
class StatisticsScreen(BaseScreen):
    ...
    def calculate_text_positions(self):
        ...
        self.text_positions = [
            [center_x, 200 + i * 40] for i in range(len(self.statistics))
        ]
    ...
    def draw(self):
        ...
        title_text = self.title_font.render("Statistics", True, (0, 0, 0))
        self.screen.blit(title_text, title_text.get_rect(center=(center_x, self.title_position[1])))

        for i, line in enumerate(self.statistics):
        ...
        if self.sidebar.visible:
            self.sidebar.draw(self.screen, "Statistics")
