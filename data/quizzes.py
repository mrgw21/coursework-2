quizzes = [
    {
        "question": "Which of the following correctly describes the way bacteria store genetic information?",
        "options": [
            {"text": "Circularly organised RNA stored within a protein capsid", "is_correct": False},
            {"text": "Circularly organised DNA stored freely within the cell", "is_correct": True},
            {"text": "Straight DNA stored freely within the cell", "is_correct": False},
            {"text": "Straight RNA stored freely within the cell", "is_correct": False},
        ],
        "hints": [
            "Bacteria do not have a nucleus, and their genetic material is stored differently from eukaryotes.",
            "Bacteria store their genetic material in a single, circular molecule of DNA."
        ]
    },
    {
        "question": "What is the primary function of macrophages in the immune system?",
        "options": [
            {"text": "Producing antibodies", "is_correct": False},
            {"text": "Engulfing and digesting pathogens", "is_correct": True},
            {"text": "Activating B cells", "is_correct": False},
            {"text": "Destroying red blood cells", "is_correct": False}
        ],
        "hints": [
            "They are part of the innate immune system and act as first responders.",
            "Their primary role is to engulf and digest foreign invaders like pathogens."
        ]
    },
    {
        "question": "What type of pathogen is known to replicate only inside host cells?",
        "options": [
            {"text": "Fungi", "is_correct": False},
            {"text": "Bacteria", "is_correct": False},
            {"text": "Viruses", "is_correct": True},
            {"text": "Parasites", "is_correct": False}
        ],
        "hints": [
            "This pathogen cannot replicate on its own.",
            "It hijacks the machinery of the host cell to produce copies of itself."
        ]
    },
    {
        "question": "What is the size range of viruses?",
        "options": [
            {"text": "1-10 μm", "is_correct": False},
            {"text": "20-800 nm", "is_correct": True},
            {"text": "2-5 mm", "is_correct": False},
            {"text": "50-100 μm", "is_correct": False}
        ],
        "hints": [
            "Viruses are much smaller than most cells.",
            "Their size is in the range of light microscopy’s limits."
        ]
    },
    {
        "question": "Where is the viral genetic material stored?",
        "options": [
            {"text": "Cell membrane", "is_correct": False},
            {"text": "Ribosome", "is_correct": False},
            {"text": "Protein Capsid", "is_correct": True},
            {"text": "Peptidoglycan Layer", "is_correct": False}
        ],
        "hints": [
            "It’s a protective structure unique to viruses.",
            "This structure plays a role in protecting the genetic material from external damage."
        ]
    },
    {
        "question": "What form does viral genetic information usually take?",
        "options": [
            {"text": "Double stranded DNA and RNA", "is_correct": False},
            {"text": "Proteins", "is_correct": False},
            {"text": "Single Stranded RNA only", "is_correct": False},
            {"text": "Both Singe and Double Stranded RNA", "is_correct": True}
        ],
        "hints": [
            "Viruses are adaptable in the way they carry genetic instructions.",
            "Some viruses require reverse transcription to replicate"
        ]
    },
    {
        "question": "What is the role of the external binding proteins on a virus?",
        "options": [
            {"text": "To aid in photosynthesis", "is_correct": False},
            {"text": "To bind to host cells", "is_correct": True},
            {"text": "To produce proteins", "is_correct": False},
            {"text": "To replicate RNA", "is_correct": False}
        ],
        "hints": [
            "These proteins determine a virus’s host specificity.",
            "They are crucial for the initial step of infection."
        ]
    },
    {
        "question": "Retroviruses, such as HIV, contain what kind of genetic material?",
        "options": [
            {"text": "Single-stranded RNA", "is_correct": True},
            {"text": "Double-stranded RNA", "is_correct": False},
            {"text": "Circular DNA", "is_correct": False},
            {"text": "Linear DNA", "is_correct": False}
        ],
        "hints": [
            "Retroviruses require reverse transcription to replicate.",
            "Their genetic material is often single-stranded."
        ]
    },
    {
        "question": "How do viruses reproduce?",
        "options": [
            {"text": "Through binary fission", "is_correct": False},
            {"text": "By spontaneous generation", "is_correct": False},
            {"text": "By using host cell machinery", "is_correct": True},
            {"text": "By dividing on their own", "is_correct": False}
        ],
        "hints": [
            "Viruses cannot replicate independently.",
            "They rely entirely on the cellular processes of their host."
        ]
    },
    {
        "question": "What is reverse transcriptase used for in retroviruses?",
        "options": [
            {"text": "To destroy host cells", "is_correct": False},
            {"text": "To convert RNA into DNA", "is_correct": True},
            {"text": "To replicate viral porteins", "is_correct": False},
            {"text": "To make ATP", "is_correct": False}
        ],
        "hints": [
            "This enzyme works against the usual flow of genetic information.",
            "It’s essential for integrating viral genetic material into the host genome."
        ]
    },
    {
        "question": "What is the typical size range of bacteria?",
        "options": [
            {"text": "0.1-1 nm", "is_correct": False},
            {"text": "10-20mm", "is_correct": False},
            {"text": "50-100μm", "is_correct": False},
            {"text": "1-2μm", "is_correct": True}
        ],
        "hints": [
            "Bacteria are larger than viruses but still microscopic.",
            "Their size can be observed under a light microscope"
        ]
    },
    {
        "question": "What structure is always present in bacterial cells?",
        "options": [
            {"text": "Flagellum", "is_correct": False},
            {"text": "Capsule", "is_correct": False},
            {"text": "Peptidoglycan cell wall", "is_correct": True},
            {"text": "Membrane Invaginations", "is_correct": False}
        ],
        "hints": [
            "It provides structural integrity to the cell.",
            "It allows the cell to withstand osmotic pressure"
        ]
    },
    {
        "question": "What is the role of ribosomes in bacteria?",
        "options": [
            {"text": "To protect against infection", "is_correct": False},
            {"text": "To synthesize proteins", "is_correct": True},
            {"text": "To store genetic material", "is_correct": False},
            {"text": "To provide structural support", "is_correct": False}
        ],
        "hints": [
            "Ribosomes are essential for translating genetic instructions.",
            "They are found in both prokaryotic and eukaryotic cells."
        ]
    },
    {
        "question": "What is the function of bacterial pili?",
        "options": [
            {"text": "Attachment to other cells", "is_correct": True},
            {"text": "Photosynthesis", "is_correct": False},
            {"text": "Movement", "is_correct": False},
            {"text": "Producing Toxins", "is_correct": False}
        ],
        "hints": [
            "Pili play a role in bacterial communication or DNA exchange.",
            "They are also involved in surface attachment."
        ]
    },
    {
        "question": "How do bacteria contribute positively to the environment?",
        "options": [
            {"text": "By causing infections", "is_correct": False},
            {"text": "By assisting in nitrogen fixation", "is_correct": True},
            {"text": "By producing toxins", "is_correct": False},
            {"text": "By absorbing sunlight", "is_correct": False}
        ],
        "hints": [
            "Some bacteria live in the soil and form symbiotic relationships with plants.",
            "They help replenish nutrients essential for plant growth."
        ]
    },
    {
        "question": "What process allows bacteria to pass additional DNA between each other?",
        "options": [
            {"text": "Binary Fission", "is_correct": False},
            {"text": "Protein synthesis", "is_correct": False},
            {"text": "Cellular respiration", "is_correct": False},
            {"text": "Horizontal transfer", "is_correct": True}
        ],
        "hints": [
            "This process helps bacteria adapt to new challenges.",
            "It often involves the sharing of plasmids."
        ]
    },
    {
        "question": "Which structure is always present in bacterial cells?",
        "options": [
            {"text": "Flagellum", "is_correct": False},
            {"text": "Ribosome", "is_correct": True},
            {"text": "Capsule", "is_correct": False},
            {"text": "Mitochondria", "is_correct": False}
        ],
        "hints": [
            "Even though bacteria lack membrane-bound organelles, they still need to make proteins.",
            "This structure is essential for translating genetic information into functional molecules."
        ]
    },
    {
        "question": "What are membrane invaginations in bacteria used for?",
        "options": [
            {"text": "Storing DNA", "is_correct": False},
            {"text": "Nitrogen Fixation and Photosynthesis", "is_correct": True},
            {"text": "Protein Synthesis", "is_correct": False},
            {"text": "Cell Division", "is_correct": False}
        ],
        "hints": [
            "Even though bacteria lack membrane-bound organelles, they still need to make proteins.",
            "This structure is essential for translating genetic information into functional molecules."
        ]
    },
   {
        "question": "What is the function of macrophages in the immune system?",
        "options": [
            {"text": "Produce antibodies", "is_correct": False},
            {"text": "Recognize and engulf pathogens", "is_correct": True},
            {"text": "Destroy red blood cells", "is_correct": False},
            {"text": "Transport oxygen", "is_correct": False}
        ],
        "hints": [
            "Macrophages are part of the body's early response to infection.",
            "Their name derives from Greek words meaning “big eaters.”"
        ]
    },
   {
        "question": "What forms after a macrophage engulfs a pathogen?",
        "options": [
            {"text": "Phagosome", "is_correct": True},
            {"text": "Lysosome", "is_correct": False},
            {"text": "Ribosome", "is_correct": False},
            {"text": "Capsid", "is_correct": False}
        ],
        "hints": [
            "This vesicle encloses the pathogen.",
            "It combines with another organelle to destroy the pathogen."
        ]
    },
   {
        "question": "What is the role of lysosomes in macrophages?",
        "options": [
            {"text": "To engulf foreign particles", "is_correct": False},
            {"text": "To store DNA", "is_correct": False},
            {"text": "To degrade pathogens with enzymes", "is_correct": True},
            {"text": "To release antibodies", "is_correct": False}
        ],
        "hints": [
            "Lysosomes play a key role in the immune response.",
            "They help break down complex molecules."
        ]
    },
   {
        "question": "Which of the following is a first line of defence against pathogens?",
        "options": [
            {"text": "Antibodies", "is_correct": False},
            {"text": "Macrophage", "is_correct": True},
            {"text": "Bacteria", "is_correct": True},
            {"text": "Viruses", "is_correct": False}
        ],
        "hints": [
            "This type of cell response to invaders without prior training",
            "They are part of the innate immune system"
        ]
    },
    {
        "question": "What does a macrophage recognize on a pathogen's surface?",
        "options": [
            {"text": "Genetic material", "is_correct": False},
            {"text": "Binding proteins", "is_correct": False},
            {"text": "Antigens", "is_correct": True},
            {"text": "Lipids", "is_correct": False}
        ],
        "hints": [
            "These molecular markers are unique to pathogens.",
            "They trigger immune responses when detected."
        ]
    },
    {
        "question": "What happens when a lysosome merges with a phagosome in a macrophage?",
        "options": [
            {"text": "The pathogen is encapsulated for storage.", "is_correct": False},
            {"text": "Enzymes degrade the pathogen.", "is_correct": True},
            {"text": "The macrophage releases antibodies.", "is_correct": False},
            {"text": "The pathogen multiplies within the vesicle.", "is_correct": False}
        ],
        "hints": [
            "The merging of these two structures forms a specialized vesicle.",
            "This step is essential for breaking down pathogens."
        ]
    },
    {
        "question": "What is the primary purpose of phagocytosis?",
        "options": [
            {"text": "To divide pathogens", "is_correct": False},
            {"text": "To create new cells", "is_correct": False},
            {"text": "To break down and eliminate pathogens", "is_correct": True},
            {"text": "To produce new macrophages", "is_correct": False}
        ],
        "hints": [
            "Phagocytosis is critical to defending the body against infections.",
            "It involves both engulfing and digesting foreign particles."
        ]
    },
    {
        "question": "Where does degradation of the pathogen occur in the macrophage?",
        "options": [
            {"text": "Cytoplasm", "is_correct": False},
            {"text": "Phagosome-lysosome complex", "is_correct": True},
            {"text": "Cell membrane", "is_correct": False},
            {"text": "Ribosome", "is_correct": False}
        ],
        "hints": [
            "This occurs in a vesicle that results from a merging process.",
            "The vesicle contains enzymes that degrade the pathogen."
        ]
    },
    {
        "question": "What is a major structural difference between bacteria and viruses?",
        "options": [
            {"text": "Bacteria have a nucleus, viruses do not.", "is_correct": False},
            {"text": "Bacteria have ribosomes, viruses do not.", "is_correct": True},
            {"text": "Viruses have mitochondria, bacteria do not.", "is_correct": False},
            {"text": "Viruses have cytoplasm, bacteria do not.", "is_correct": False}
        ],
        "hints": [
            "Viruses rely entirely on host cells for protein synthesis.",
            "Ribosomes are essential for producing proteins."
        ]
    },
    {
        "question": "How do viruses differ from bacteria in terms of reproduction?",
        "options": [
            {"text": "Viruses divide by binary fission.", "is_correct": False},
            {"text": "Viruses require a host cell to replicate.", "is_correct": True},
            {"text": "Bacteria must enter a host cell to reproduce.", "is_correct": False},
            {"text": "Bacteria replicate by injecting their DNA into a host.", "is_correct": False}
        ],
        "hints": [
            "Viruses cannot perform independent replication.",
            "Bacteria do not need host cells to divide."
        ]
    },
    {
        "question": "Which of these structures is found in bacteria but not viruses?",
        "options": [
            {"text": "Protein capsid", "is_correct": False},
            {"text": "Cell wall", "is_correct": True},
            {"text": "RNA", "is_correct": False},
            {"text": "Lipid envelope", "is_correct": False}
        ],
        "hints": [
            "This structure is essential for maintaining bacterial internal organization.",
            "It surrounds the cytoplasm in bacteria but is absent in viruses."
        ]
    },
    {
        "question": "Which of the following best describes the size comparison between viruses and bacteria?",
        "options": [
            {"text": "Bacteria and viruses are about the same size.", "is_correct": False},
            {"text": "Bacteria are generally smaller than viruses.", "is_correct": False},
            {"text": "Viruses are much smaller than bacteria.", "is_correct": True},
            {"text": "Some bacteria are smaller than viruses.", "is_correct": False}
        ],
        "hints": [
            "Bacteria are typically visible under light microscopes.",
            "Viruses require electron microscopes for visualization."
        ]
    },
    {
        "question": "Which component is used by viruses to attach to host cells?",
        "options": [
            {"text": "Flagella", "is_correct": False},
            {"text": "Ribosomes", "is_correct": False},
            {"text": "Spike proteins", "is_correct": True},
            {"text": "Cytoplasm", "is_correct": False}
        ],
        "hints": [
            "These structures are external and bind to host cell receptors.",
            "They are found on the outer surface of viruses, not bacteria."
        ]
    },
    {
        "question": "What is a defining feature of bacterial genetic material?",
        "options": [
            {"text": "It is linear and enclosed in a nucleus.", "is_correct": False},
            {"text": "It is circular and located in the cytoplasm.", "is_correct": True},
            {"text": "It is single-stranded and located in the capsid.", "is_correct": False},
            {"text": "It is double-stranded RNA.", "is_correct": False}
        ],
        "hints": [
            "Bacteria lack nuclei to contain their genetic material.",
            "Their DNA is compact and often forms a ring."
        ]
    },
    {
        "question": "What process enables bacteria to rapidly spread antibiotic resistance within populations?",
        "options": [
            {"text": "Binary fission", "is_correct": False},
            {"text": "Horizontal gene transfer", "is_correct": True},
            {"text": "Host-cell hijacking", "is_correct": False},
            {"text": "Capsid formation", "is_correct": False}
        ],
        "hints": [
            "This process spreads antibiotic resistance among bacteria.",
            "It does not involve reproduction."
        ]
    },
    {
        "question": "Why are viruses not considered living organisms?",
        "options": [
            {"text": "They do not have genetic material.", "is_correct": False},
            {"text": "They cannot replicate autonomously.", "is_correct": True},
            {"text": "They lack a cell membrane.", "is_correct": False},
            {"text": "They cannot move independently.", "is_correct": False}
        ],
        "hints": [
            "Viruses depend entirely on the machinery of host cells.",
            "Life is generally defined by independent metabolism and reproduction."
        ]
    },
    {
        "question": "How do viruses deliver their genetic material into a host cell?",
        "options": [
            {"text": "By binary fission", "is_correct": False},
            {"text": "By endocytosis or injection", "is_correct": True},
            {"text": "By releasing toxins", "is_correct": False},
            {"text": "By using ribosomes", "is_correct": False}
        ],
        "hints": [
            "Not all the components of viruses enter a cell upon infection.",
            "Some viruses use binding proteins to attach to cells and inject their genome."
        ]
    },
    {
        "question": "Why are bacteria essential for ecosystems, unlike viruses?",
        "options": [
            {"text": "They help recycle nutrients through decomposition.", "is_correct": True},
            {"text": "They infect only specific species.", "is_correct": False},
            {"text": "They cause genetic mutations in plants.", "is_correct": False},
            {"text": "They protect other organisms from viruses.", "is_correct": False}
        ],
        "hints": [
            "Think about processes like nitrogen fixation.",
            "Bacteria play a key role in soil and nutrient cycles."
        ]
    },
    {
        "question": "How do bacteria differ from viruses in causing infections?",
        "options": [
            {"text": "Bacteria rely on injecting their DNA into host cells.", "is_correct": False},
            {"text": "Bacteria can multiply outside host cells, viruses cannot.", "is_correct": True},
            {"text": "Both need to hijack host cell machinery for reproduction.", "is_correct": False},
            {"text": "Both always release toxins to infect the host.", "is_correct": False}
        ],
        "hints": [
            "Think about which organism can multiply in nutrient-rich environments.",
            "Viruses are obligate parasites, while bacteria are not."
        ]
    }
    # Add more quizzes...
]